...
class DailyStatJob:
    """
        Class for the Carrot driving_daily_stat Job
    """

    def __init__(self, spark_session, load_type="INCREMENTAL") -> None:
        """
            Initializer for driving_daily_stat class object
        """
    ...
    def _extract_new_partition_cols(self, dataframe):
        ...
        if not dataframe.rdd.isEmpty():
            ...
            df_distinct = dataframe.select(dataframe.year, dataframe.month, dataframe.day).distinct()
            distinct_part_cols = df_distinct.collect()

            for part in distinct_part_cols:
                partition_paths.append(
                    f"year={part['year']}/month={part['month']}/day={part['day']}/")
    ...
    def _read_target_latest_part_cols(self, bucket, prefix):
        try:
            ...
            df.createOrReplaceTempView("daily_stat_df_tmp")
    ...
    def extract_latest_part_cols(self):
        latest_source_part_cols = self._read_target_latest_part_cols(
            self.parameters.gold_data_bucket_path, self.parameters.driving_daily_stat_gold_table_name
        )
    ...
    def _read_data_from_s3(self, bucket_name, table_name, target_partition_datetime):
        ...
        if self.load_type == "INCREMENTAL":
            ...
            df = df.withColumn("partition_datetime", F.to_timestamp(
                F.concat(df.year, F.lit("-"), df.month, F.lit("-"), df.day, F.lit(" "), F.lit("00"), F.lit(":"),
    ...
    def apply_speed_filter_detection_formula(self, agg_df):
        try:
            ...
            original_df = agg_df
            list_data = agg_df.select("date", "day", "trip_id", "latitude", "longitude", "sp", "ct").collect()
            ...
            for data in list_data:
                ...
                grouped_calculated_list_data.append(
                    {'date': data['date'], 'day': data['day'], 'trip_id': data['trip_id'],
            ...
            final_df = original_df.join(
                calculated_df, (original_df.date == calculated_df.date) & (original_df.day == calculated_df.day)
    ...
    def refresh_daily_stat_table(self):
        """
            Main driver method for driving_daily_stat table for Gold Layer
        """
        ...
        try:
            self.cw_manager.log_info(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                     ...
                                     f"Started Gold Layer Job for {self.parameters.driving_daily_stat_gold_table_name}"
            ...
            history_exists = self._check_target_catalog_and_data_exist(
                ...
                self.parameters.driving_daily_stat_gold_table_name)
            ...
            if self.load_type == "INCREMENTAL":
                self.cw_manager.log_info(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                         ...
                                         f"in {self.parameters.driving_daily_stat_gold_table_name} table")
            ...
            if not position_df.rdd.isEmpty():
                self.cw_manager.log_info(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                         ...
                                         f"{self.parameters.driving_daily_stat_gold_table_name} table for Gold Layer.")
                ...
                self.cw_manager.log_info(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                         ...
                                         f"{self.parameters.driving_daily_stat_gold_table_name} table for Gold Layer.")
                daily_stat_df = position_df.select(
                    position_df.trip_id,
                    position_df.user_id,
                ...
                daily_stat_df = daily_stat_df.withColumn('ts', F.to_timestamp(daily_stat_df.ts, 'yyyy-MM-dd HH:mm:ss'))
                daily_stat_df = daily_stat_df.withColumn('date', F.to_date(daily_stat_df.ts))
                daily_stat_df = daily_stat_df.withColumn('day', F.dayofmonth(daily_stat_df.ts))

                # Group By on date and day for daily stats
                ...
                agg_daily_stat_df = daily_stat_df.orderBy(
                    daily_stat_df.date, daily_stat_df.day, daily_stat_df.trip_id, daily_stat_df.ct)\
                    .groupBy(daily_stat_df.date, daily_stat_df.day, daily_stat_df.trip_id).agg(
                ...
                agg_daily_stat_df = agg_daily_stat_df.withColumn("size_of_latitude", F.size(agg_daily_stat_df.latitude))

                agg_daily_stat_df = agg_daily_stat_df.withColumn("total_distance", self.apply_distance_formula(
                    agg_daily_stat_df.latitude,
                    agg_daily_stat_df.longitude,
                    agg_daily_stat_df.size_of_latitude
                ))

                agg_daily_stat_df = agg_daily_stat_df.withColumn(
                    "total_distance", F.round(agg_daily_stat_df.total_distance).cast('long'))

                new_agg_daily_stat_df = self.apply_speed_filter_detection_formula(agg_daily_stat_df)

                new_agg_daily_stat_df = new_agg_daily_stat_df.groupBy(
                    new_agg_daily_stat_df.date, new_agg_daily_stat_df.day).agg(
                    F.countDistinct("trip_id").alias("trip_count"),
                    F.sum("user_count").alias("user_count"),
                    F.sum("vehicle_count").alias("vehicle_count"),
                    F.sum(new_agg_daily_stat_df.total_driving_time).alias("total_driving_time"),
                    F.sum(new_agg_daily_stat_df.total_distance).alias("total_distance"),
                    F.sum(new_agg_daily_stat_df.total_sudden_start_count).alias("total_sudden_start_count"),
                    F.sum(new_agg_daily_stat_df.total_sudden_stop_count).alias("total_sudden_stop_count"),
                    F.sum(new_agg_daily_stat_df.total_acceleration_count).alias("total_acceleration_count"),
                    F.sum(new_agg_daily_stat_df.total_deceleration_count).alias("total_deceleration_count")
                )

                new_agg_daily_stat_df = new_agg_daily_stat_df.withColumn(
                    'total_driving_time', F.round(new_agg_daily_stat_df.total_driving_time / 60).cast('long'))

                # Add remaining partition columns to the DataFrame
                agg_df_with_partition_cols = \
                    new_agg_daily_stat_df.withColumn("year", F.year("date"))
                agg_df_with_partition_cols = \
                    agg_df_with_partition_cols.withColumn("month", F.month("date"))

                final_daily_stat_df = agg_df_with_partition_cols.select(
                    "date", "day", "trip_count", "user_count", "vehicle_count", "total_driving_time",
                ...
                final_daily_stat_df = final_daily_stat_df.withColumn(
                    "avg_distance", final_daily_stat_df.total_distance / final_daily_stat_df.trip_count)
                final_daily_stat_df = final_daily_stat_df.withColumn(
                    "avg_driving_time", final_daily_stat_df.total_driving_time / final_daily_stat_df.trip_count)
                final_daily_stat_df = final_daily_stat_df.withColumn(
                    "avg_sudden_start_count",
                    final_daily_stat_df.total_sudden_start_count / final_daily_stat_df.trip_count)
                final_daily_stat_df = final_daily_stat_df.withColumn(
                    "avg_sudden_stop_count",
                    final_daily_stat_df.total_sudden_stop_count / final_daily_stat_df.trip_count)
                final_daily_stat_df = final_daily_stat_df.withColumn(
                    "avg_acceleration_count",
                    final_daily_stat_df.total_acceleration_count / final_daily_stat_df.trip_count)
                final_daily_stat_df = final_daily_stat_df.withColumn(
                    "avg_deceleration_count",
                    final_daily_stat_df.total_deceleration_count / final_daily_stat_df.trip_count)
                final_daily_stat_df = final_daily_stat_df.withColumn(
                    "year_month",
                    F.concat(final_daily_stat_df.year, final_daily_stat_df.month))
                ...
                self.cw_manager.log_info(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                         ...
                                         f"{self.parameters.driving_daily_stat_gold_table_name} table.")
                # # write data to s3 and update catalog
                self._write_data_to_s3(final_daily_stat_df, self.parameters.gold_data_bucket_path,
                                       self.parameters.driving_daily_stat_gold_table_name)
                ...
                partition_paths = self._extract_new_partition_cols(final_daily_stat_df)
                ...
                self.cw_manager.log_info(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                         ...
                                         f"{self.parameters.driving_daily_stat_gold_table_name} data on ATHENA "
                                         f"started")

                self.athena_helper.catalog_driving_daily_stat_table(
                    self.parameters.gold_data_bucket_path, partition_paths, self.parameters.glue_database_gold_name,
                    self.parameters.driving_daily_stat_gold_table_name, self.load_type)
                ...
                self.cw_manager.log_info(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                         ...
                                         f"table name: {self.parameters.driving_daily_stat_gold_table_name}")
            ...
            self.cw_manager.log_error(self.parameters.CW_LOG_STREAM_GOLD + '/'.join([str(current_datetime.year),
                                                                                     ...
                                                                                     str(current_datetime.hour),
                                                                                     str(current_datetime.minute)]),
                                      f"Failed Gold Layer Job for {self.parameters.driving_daily_stat_gold_table_name}"
                                      f" table.")
            ...
            sns_subject = f"GOLD Layer Job Failed for Table: {self.parameters.driving_daily_stat_gold_table_name} " \
                          f"| Carrot Datalake"
            sns_message = "This SNS is to notify that the Batch ETL Job from Carrot Data lake for GOLD Layer's " \
                          f"{self.parameters.driving_daily_stat_gold_table_name} table has FAILED. \n" \
...
if __name__ == "__main__":
    ...
    if args.get('LOAD_TYPE', 'INCREMENTAL') in ['FULL_LOAD', 'INCREMENTAL']:
        stream_driver = DailyStatJob(spark, load_type=arg_load_type)
        stream_driver.refresh_daily_stat_table()
    else:
